...
class ViT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(self.pos_embedding.shape[-1], self.num_h, self.num_c, cls_token=True)
        self.pos_embedding.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], self.num_h, self.num_c, cls_token=True)
    ...
    def forward_encoder(self, input, mask_ratio=0.75):
    ...
    def forward(self, input, mask_ratio=0.75):
