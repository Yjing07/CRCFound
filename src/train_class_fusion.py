...
def get_weight(args, lab_dict):
    ...
    weight = torch.tensor([(sum(class_count)-i)/sum(class_count) for i in class_count])
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if val_auc > val_best:
            _all = {}
            assert len(labels_val) == len(pres_val) == len(ids_val)
            _all['ids'] = ids_val
            _all['preds'] = [int(part) for part in pres_val]
            _all['labels'] = [int(part) for part in labels_val]
